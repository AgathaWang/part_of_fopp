
.. mchoice:: TransformingSequences_WPAccumulatorPatternStrategies_question8_11_9
    :prim_comp: DT01, ITER05
    :supp_comp: ITER01
    :answer_a: string
    :answer_b: list
    :answer_c: integer
    :answer_d: none, there is no accumulator variable.
    :feedback_a: Incorrect, that is not the best type for the accumulator variable.
    :feedback_b: Incorrect, that is not the best type for the accumulator variable.
    :feedback_c: Yes, because we want to keep track of a number.
    :feedback_d: Incorrect, we will need an accumulator variable.
    :correct: c
    :practice: T

    What type should be used for the accumulator variable in the following prompt? 
    Write code to sum up all of the numbers in the list ``seat_counts``. 
    Store that number in the variable ``total_seat_counts``.
