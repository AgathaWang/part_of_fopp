.. mchoice:: TransformingSequences_WPAccumulatorPatternStrategies_question8_11_11
    :prim_comp: ITER05, ITER06
    :supp_comp: ITER01
    :answer_a: string
    :answer_b: list
    :answer_c: integer
    :answer_d: none, there is no accumulator variable.
    :feedback_a: Incorrect, there should not be an accumulator variable.
    :feedback_b: Incorrect, there should not be an accumulator variable.
    :feedback_c: Incorrect, there should not be an accumulator variable.
    :feedback_d: Correct, because this prompt does not require an accumulator pattern
    :correct: d
    :practice: T

    What type should be used for the accumulator variable in the following prompt? Write code to print 
    out each character of the string ``my_str`` on a separate line.
